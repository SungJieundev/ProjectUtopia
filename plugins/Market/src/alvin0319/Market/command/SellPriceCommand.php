...
final class SellPriceCommand extends Command implements PluginOwned{
	use PluginOwnedTrait;

	public function __construct(){
		parent::__construct("판매가", "아이템의 판매가를 설정합니다.", "/sellprice <price> <currency>", ["sp"]);
		$this->setPermission("market.command.sellprice");
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args) : void{
		...
		if(!is_numeric($price) || ($price = (int) $price) < -1){
			$sender->sendMessage(Loader::$prefix . "판매가는 정수여야 합니다.");
			return;
		}
		...
		if($item->isNull()){
			$sender->sendMessage(Loader::$prefix . "공기의 판매가를 설정할 수 없습니다.");
			return;
		}
		$currency = EconomyAPI::getInstance()->getCurrency($currencyName);

		$market = Loader::getInstance()->getMarketManager()->getMarketByItem($item) ?? Loader::getInstance()->getMarketManager()->createMarket($item, -1, $price, $currency);
		$market->setSellPrice($price);
		$market->setCurrency($currency);
		$sender->sendMessage(Loader::$prefix . "{$item->getName()}의 구매가를 {$currency->format($price)}으(로) 설정했습니다.");
	}
}
